...
public class ApiResource {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public Category booksCategoryName(@PathParam("category-name") String categoryName,
                                          @Context HttpServletRequest httpRequest) {
        try {
...
//            return bookDao.findByCategoryId(category.getCategoryId());
    ...
    public List<Book> booksByCategoryName(@PathParam("category-name") String categoryName,
        ...
        try {
            ...
            throw new ApiException(String.format("Books lookup by category-id %d failed", categoryName), e);
        }
    }
    @GET
